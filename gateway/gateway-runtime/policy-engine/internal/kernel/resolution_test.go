...
// ─── Attribute bounding at the kernel boundary ───────────────────────────────

// Attribute values originate in the request, so the bound is a security control rather
// than tidiness: an unbounded caller-chosen value copied into every request's context is
// a memory cost, and a cardinality denial of service if a policy ever labels a metric
// with it.
func TestBoundResolutionAttributes_DropsOverLongValues(t *testing.T) {
	long := strings.Repeat("a", maxResolutionAttributeValueLen+1)
	bounded := boundResolutionAttributes("mcp", map[string]string{
		"mcp.body.method":          "tools/call",
		"mcp.body.capability.name": long,
	})

	assert.Equal(t, map[string]string{
		"mcp.body.method": "tools/call",
	}, bounded)
	assert.NotContains(t, bounded, "mcp.body.capability.name",
		"dropped, never truncated: a shortened tool name that still looks valid is worse than an absent one")
}

func TestBoundResolutionAttributes_CapsCountDeterministically(t *testing.T) {
	attrs := make(map[string]string, maxResolutionAttributes+10)
	for i := range maxResolutionAttributes + 10 {
		attrs[fmt.Sprintf("mcp.body.k%03d", i)] = "v"
	}

	first := boundResolutionAttributes("mcp", attrs)
	second := boundResolutionAttributes("mcp", attrs)

	assert.Len(t, first, maxResolutionAttributes)
	assert.Equal(t, first, second,
		"selection is sorted, so an over-count drops the same keys on every request")
}

func TestBoundResolutionAttributes_EmptyStaysNil(t *testing.T) {
	assert.Nil(t, boundResolutionAttributes("route-key", nil))
	assert.Nil(t, boundResolutionAttributes("route-key", map[string]string{}))
}
