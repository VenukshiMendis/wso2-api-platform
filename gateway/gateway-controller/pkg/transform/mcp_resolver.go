...
// mcpResolverOperation is the operation component of every MCP chain key.
//
// It is a constant because an MCP proxy has exactly one policy chain regardless of which
// tool is invoked — the configuration model has no way to express anything else:
// MCPProxyConfigData.Policies is a single proxy-level list, MCPTool carries no policies,
// and Policy has no path scoping. Per-tool behaviour lives inside a policy's own params
// (mcp-authz's "tools:" list), so the resolver's job is to tell one chain what the
// request is, not to choose between chains.
//
// Making chains per-operation later means changing this and nothing else here.
const mcpResolverOperation = "mcp"

// MaxRequestBodyBytes is deliberately NOT set on the MCP route.
//
// The engine already applies DefaultMaxResolverRequestBodyBytes (64 KiB) to any
// body-resolved route that carries no explicit limit, and that default exists precisely
// so the ceiling can be tightened in one place — it guards a body parsed before any
// authentication policy has run. Pinning the same number from here would put a redundant
// field on the wire and quietly exempt MCP from a future change to it.
//
// Set one here only to choose a value deliberately different from the engine default.

// mcpResolutionApplies reports whether this config is an MCP proxy, and therefore whether
// its multiplexed route should carry the MCP resolver.
//
// No resolver configuration accompanies it. The engine's MCP resolver reads none — every
// MCP route parses the body, and nothing on the wire can change that — so emitting any
// would be a field written and never read.
//
// It reads SourceConfiguration rather than Configuration because by the time a
// transformer runs, an MCP proxy has already been desugared into a RestAPI, and the
// original configuration is what identifies it as one.
...
// already resolved.
//
// The policy cannot work this out for itself. PolicyMetadata carries nothing about the
// route's resolver, and ProcessingMode is fixed at chain-build time, so the only way a
// policy can decline the body on a resolver-bearing route and still ask for it elsewhere is
// for the controller to state which kind of route this is.
//
// Absence is the meaningful default. A controller too old to have a resolver injects
// nothing, each policy defaults the parameter to false, and buffers and parses the body as
// it always did — so a new policy paired with an old gateway keeps working rather than
// losing every configured auth exemption.
//
// It overwrites rather than defaults: the value is the gateway's to state, and an
// operator-supplied one would be a second source of truth for a fact only the controller
// knows. Policies outside the set are untouched.
